...
public class TESTAPIFinalClient: AWSAPIGatewayClient {

	static let AWSInfoClientKey = "TESTAPIFinalClient"

	private static let _serviceClients = AWSSynchronizedMutableDictionary()
	private static let _defaultClient:TESTAPIFinalClient = {
        ...
        return TESTAPIFinalClient(configuration: serviceConfiguration!)
	}()
	...
	/**
	 ...
	 Then call the following to get the default service client:
	     ...
	     let serviceClient = TESTAPIFinalClient.default()

     Alternatively, this configuration could also be set in the `info.plist` file of your app under `AWS` dictionary with a configuration dictionary by name `TESTAPIFinalClient`.
	
	 @return The default service client.
	 */ 
	 
	public class func `default`() -> TESTAPIFinalClient{
	...
	/**
	 ...
	 If you want to enable AWS Signature, set the default service configuration in `func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?)`
	     ...
	     func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
	         ...
	         TESTAPIFinalClient.registerClient(withConfiguration: configuration, forKey: "USWest2TESTAPIFinalClient")
	 ...
	 Then call the following to get the service client:
	     ...
	     let serviceClient = TESTAPIFinalClient.client(forKey: "USWest2TESTAPIFinalClient")
	...
	public class func registerClient(withConfiguration configuration: AWSServiceConfiguration, forKey key: String){
		_serviceClients.setObject(TESTAPIFinalClient(configuration: configuration), forKey: key  as NSString);
	}
	 ...
	 */
	public class func client(forKey key: String) -> TESTAPIFinalClient {
		objc_sync_enter(self)
		if let client: TESTAPIFinalClient = _serviceClients.object(forKey: key) as? TESTAPIFinalClient {
			objc_sync_exit(self)
		    return client
		}
		...
		if let serviceInfo = serviceInfo {
			...
			TESTAPIFinalClient.registerClient(withConfiguration: serviceConfiguration!, forKey: key)
		}
		objc_sync_exit(self)
		return _serviceClients.object(forKey: key) as! TESTAPIFinalClient;
	}
	...
	init(configuration: AWSServiceConfiguration) {
	    ...
	    var URLString: String = "https://bvqzxkbm1f.execute-api.us-east-1.amazonaws.com/prod"
    ...
    /*
     ...
     return type: Empty
     */
    public func loginCredentialsGet() -> AWSTask<Empty> {
	    ...
	    return self.invokeHTTPRequest("GET", urlString: "/loginCredentials", pathParameters: pathParameters, queryParameters: queryParameters, headerParameters: headerParameters, body: nil, responseClass: Empty.self) as! AWSTask<Empty>
	}
